...
import { window, column, row, text, container } from '../src/ui/index.js'
import { Canvas } from '../src/ui/widgets/canvas.js'
import {
  rect, circle, path, moveTo, lineTo, close, stroke, linearGradient,
...
function hsvToHex(h: number, s: number, v: number): string {
  ...
  if (hSector < 1) { r1 = c; g1 = x; b1 = 0 }
  else if (hSector < 2) { r1 = x; g1 = c; b1 = 0 }
  else if (hSector < 3) { r1 = 0; g1 = c; b1 = x }
  else if (hSector < 4) { r1 = 0; g1 = x; b1 = c }
  else if (hSector < 5) { r1 = x; g1 = 0; b1 = c }
  else { r1 = c; g1 = 0; b1 = x }
  ...
  return [
    rect(SQ_ORIGIN, SQ_ORIGIN, SQ_SIZE, SQ_SIZE, {
      fill: linearGradient(
        ...
        [{ offset: 0.0, color: "#ffffff" }, { offset: 1.0, color: hueColor }],
      ),
    }),
  ]
}
...
function inSquare(x: number, y: number): boolean {
  return x >= SQ_ORIGIN && x <= SQ_ORIGIN + SQ_SIZE &&
