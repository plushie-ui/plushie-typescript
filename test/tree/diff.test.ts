...
function w(id: string, type: string, props: Record<string, unknown> = {}, children: WireNode[] = []): WireNode {
...
describe("diff", () => {
  ...
  test("identical trees produce no ops", () => {
    const tree = w("root", "column", { spacing: 8 }, [
      w("a", "text", { content: "hi" }),
    ])
  ...
  test("nested prop change has correct path", () => {
    const old = w("root", "column", {}, [
      w("row", "row", {}, [
        w("btn", "button", { label: "old" }),
      ]),
    ])
    const now = w("root", "column", {}, [
      w("row", "row", {}, [
        w("btn", "button", { label: "new" }),
      ]),
    ])
    const ops = diff(old, now)
    expect(ops).toHaveLength(1)
