  ...
  test("reports a failing artifact build even when a later child succeeds", async () => {
    ...
    writeExecutable(
      ...
      [
        ...
        "echo bin > target/release/plushie-renderer",
      ].join("\n"),
    );
  ...
  test("builds WASM when the native widget build replaces the stock binary build", async () => {
    ...
    writeExecutable(
      ...
      [
        ...
        '    : > "$PLUSHIE_CARGO_MARKER"',
        "    mkdir -p node_modules/.plushie/renderer-spec/target/plushie-renderer/target/release",
        "    echo bin > node_modules/.plushie/renderer-spec/target/plushie-renderer/target/release/plushie-renderer-spec-renderer",
        "    ;;",
        "  *)",
