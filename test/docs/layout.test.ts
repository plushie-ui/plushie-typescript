...
import {
  window, column, row, container, text, button, image,
  scrollable, stack, space, grid,
...
test("layout_length_fill", () => {
  const tree = normalize(
    column({ id: "main", width: "fill" }, [text("x", "x")])
  )
...
test("layout_length_fixed", () => {
  const tree = normalize(
    container("sidebar", { width: 250 }, [text("x", "x")])
  )
...
test("layout_padding_uniform", () => {
  const tree = normalize(
    container("box", { padding: 16 }, [text("x", "x")])
  )
...
test("layout_padding_xy", () => {
  const tree = normalize(
    container("box", { padding: [8, 16] }, [text("x", "x")])
  )
